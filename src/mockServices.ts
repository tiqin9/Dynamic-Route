...
class MockMFSAPI {
   ...
   */
  async getHVOrderData(orderId: string): Promise<MFSData> {
    // Simulate API delay
   ...
   */
  async getHVOrdersData(orderIds: string[]): Promise<Map<string, MFSData>> {
    const results = new Map<string, MFSData>();
...
class MockServiceCenterAPI {
   ...
   */
  async getAvailableSlots(date: Date, locationId: string): Promise<ServiceCenterData> {
    await new Promise(resolve => setTimeout(resolve, 100));
   ...
   */
  async schedulePickup(orderId: string, pickupTime: Date): Promise<boolean> {
    await new Promise(resolve => setTimeout(resolve, 100));
   ...
   */
  async getReturnInventory(): Promise<ServiceCenterData> {
    await new Promise(resolve => setTimeout(resolve, 100));
   ...
   */
  async confirmReturn(orderId: string, timestamp: Date): Promise<boolean> {
    await new Promise(resolve => setTimeout(resolve, 50));
...
export function generateMockOrders(count: number = 50): Order[] {
  ...
  const types: Array<'HV' | 'HVB' | 'VOR' | 'MRB'> = ['HV', 'HVB', 'VOR', 'MRB'];
  const orders: Order[] = [];
 ...
 */
export function generateRealisticMilkRuns(): Order[] {
  const orders: Order[] = [];
  let orderId = 1;
